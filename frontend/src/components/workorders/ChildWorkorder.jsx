...
import "./css/createworkorder.css"; // ✅ Your existing CSS
...
const ChildWorkOrder = () => {
  ...
  const [images, setImages] = useState({}); // ✅ multiple images per type
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        Swal.fire({
          title: "⚠️ Fetch Error",
          text: "Failed to load work order setup data.",
          icon: "error",
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedTypes.length === 0) {
      Swal.fire({
        ...
        text: "Please select a Work Order Type before creating.",
        icon: "warning",
        confirmButtonColor: "#3085d6",
      });
      return;
    ...
    if (!selectedArea) {
      Swal.fire({
        title: "⚠️ Missing Information",
        text: "Please select a Work Order Area before creating.",
        icon: "warning",
        confirmButtonColor: "#3085d6",
      });
      return;
    ...
    if (!selectedClient) {
      Swal.fire({
        title: "⚠️ Missing Information",
        text: "Please select a Client before creating.",
        icon: "warning",
        confirmButtonColor: "#3085d6",
      });
      return;
    ...
    try {
      ...
      Swal.fire({
        title: "❌ Error",
        text: err.message || "Something went wrong while creating workorder.",
        icon: "error",
        confirmButtonColor: "#3085d6",
      });
    }
